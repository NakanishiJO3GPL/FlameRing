...
const PAN_ON_TH: u16 = 500;
const PAN_OFF_TH: u16 = 3000;
const MAX_DUTY_MICRO: u64 = 5_000;
...
fn update_state(state: State, level: &mut u8) -> State {
    match state {
        ...
        State::PowerOn => match CHANNEL.try_receive() {
            Ok(Event::ButtonPressed(ButtonKind::Power)) => State::PowerOff,
            Ok(Event::ButtonPressed(ButtonKind::Weak)) => State::LevelDown,
            Ok(Event::ButtonPressed(ButtonKind::Strong)) => State::LevelUp,
...
fn nikomi_animation(
    ...
    let freq0: f32 = (time_stamp).sin() + 0.5;
    let freq1: f32 = (time_stamp * 2.0).sin() + 0.5;
    let freq2: f32 = (time_stamp * 3.8).sin() + 0.5;
...
async fn level_change_animation(
    ...
    for duty_percent in (0..=100).skip(5) {
        let duty = core_time::Duration::from_micros(MAX_DUTY_MICRO * duty_percent as u64 / 100);
        pwm0.write(duty);
        em_time::Timer::after(em_time::Duration::from_millis(5)).await;
    }
    for duty_percent in (0..=100).rev().skip(5) {
        let duty = core_time::Duration::from_micros(MAX_DUTY_MICRO * duty_percent as u64 / 100);
        pwm0.write(duty);
        em_time::Timer::after(em_time::Duration::from_millis(5)).await;
    }
